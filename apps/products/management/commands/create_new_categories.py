"""
Створює нову структуру категорій згідно нової схеми
"""
from django.core.management.base import BaseCommand
from django.utils.text import slugify
from apps.products.models import Category


class Command(BaseCommand):
    help = 'Створює нову структуру категорій'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('Створення нової структури категорій...'))
        
        # Структура категорій з іконками та типами
        categories_structure = [
            {
                'name': 'Секс-іграшки',
                'slug': 'sex-toys',
                'icon': '🎀',
                'type': 'general',
                'children': [
                    'Вібратори',
                    'Фалоімітатори',
                    'Мастурбатори і вагіни',
                    'Смарт-іграшки',
                    'Вакуумні стимулятори',
                    'Анальні іграшки',
                    'Вагінальні кульки',
                    'Страпони',
                    'Насадки та ерекційні кільця',
                    'Помпа для члена',
                    'Секс-ляльки',
                    'Секс машини',
                    'Секс меблі',
                ]
            },
            {
                'name': 'Білизна, костюми',
                'slug': 'underwear-costumes',
                'icon': '👙',
                'type': 'general',
                'children': [
                    'Комплекти',
                    'Боді',
                    'Корсети',
                    'Пеньюари і сорочки',
                    'Еротичні бодістокінги і костюми-сітка',
                    'Лакована білизна',
                    'Гартери, чокери, портупеї',
                    'Рольові костюми',
                ]
            },
            {
                'name': 'БДСМ, Фетиш',
                'slug': 'bdsm-fetish',
                'icon': '🔗',
                'type': 'general',
                'children': [
                    'Набори іграшок',
                    'Маски, пов\'язки',
                    'Кляпи',
                    'Нашийники, повідці',
                    'Батоги, стеки, флогери, ляскалки',
                    'Затискачі для сосків та грудей',
                    'Свічки для BDSM',
                ]
            },
            {
                'name': 'Прелюдія',
                'slug': 'foreplay',
                'icon': '📅',
                'type': 'general',
                'children': [
                    'Стимулятори для клітора',
                    'Рідкий вібратор',
                    'Пролонгатори для чоловіків',
                    'Косметика з феромонами',
                    'Засоби для оральних ласк',
                    'Стимулятори для точки G',
                    'Стимулятори для пеніса',
                    'Масажні свічки',
                    'Класичні масла на масляній основі',
                    'Масажні пінки',
                    'Стимулятори для сосків',
                    'Звужуючі засоби',
                    'Для інтимної гігієни',
                ]
            },
            {
                'name': 'Лубриканти',
                'slug': 'lubricants',
                'icon': '🧴',
                'type': 'general',
                'children': [
                    'Вагінальні',
                    'Анальні',
                    'Оральні (їстівні)',
                    'Пробники',
                    'Для мастурбаторів',
                ]
            },
            {
                'name': 'Сексуальне здоров\'я',
                'slug': 'sexual-health',
                'icon': '🩺',
                'type': 'general',
                'children': [
                    'Масажери простати',
                    'Тренажери Кегеля',
                    'Екстендери (збільшення члена)',
                    'Вакуумні помпи, гідропомпи',
                    'Вагінальні кульки',
                ]
            },
            {
                'name': 'Жінкам',
                'slug': 'for-women',
                'icon': '♀',
                'type': 'women',
                'children': [
                    'Вібратори',
                    'Віброкулі',
                    'Кролики',
                    'Для пар (We-Vibe)',
                    'Вакуумні',
                    'Зона G',
                    'Вагінально-кліторальні',
                    'Фалоімітатори з вібрацією',
                    'Реалістичні',
                    'Віброяйця',
                    'Hi-tech іграшки',
                    'Пульсатори',
                ]
            },
            {
                'name': 'Чоловікам',
                'slug': 'for-men',
                'icon': '♂',
                'type': 'men',
                'children': [
                    'Вагіни (реалістичні мастурбатори)',
                    'Покет-мастурбатори',
                    'З вібрацією і Hi-Tech',
                    'Масажери простати з вібрацією',
                    'Вакуумні помпи',
                    'Гідропомпи',
                    'Екстендери',
                ]
            },
            {
                'name': 'Для двох',
                'slug': 'for-couples',
                'icon': '💑',
                'type': 'couple',
                'children': [
                    'На подарунок',
                    'Секс ігри для пари',
                    'Для закоханих',
                    'Вібратори',
                    'Фалоімітатори',
                    'Страпони',
                    'Лубриканти',
                    'Прелюдія',
                    'Смарт-іграшки',
                ]
            },
        ]
        
        created_count = 0
        
        for cat_data in categories_structure:
            # Створюємо головну категорію
            parent_slug = cat_data.get('slug', slugify(cat_data['name']))
            parent, created = Category.objects.get_or_create(
                slug=parent_slug,
                defaults={
                    'name': cat_data['name'],
                    'icon': cat_data['icon'],
                    'category_type': cat_data['type'],
                    'is_active': True,
                    'sort_order': created_count,
                }
            )
            
            if created:
                self.stdout.write(f'  ✓ Створено: {parent.name} ({parent.icon})')
                created_count += 1
            else:
                # Оновлюємо іконку та тип
                parent.icon = cat_data['icon']
                parent.category_type = cat_data['type']
                parent.save()
                self.stdout.write(f'  ↻ Оновлено: {parent.name}')
            
            # Створюємо підкатегорії
            for idx, child_name in enumerate(cat_data.get('children', [])):
                child_slug = slugify(f"{parent_slug}-{child_name}")
                child, child_created = Category.objects.get_or_create(
                    slug=child_slug,
                    defaults={
                        'name': child_name,
                        'parent': parent,
                        'is_active': True,
                        'sort_order': idx,
                        'category_type': cat_data['type'],
                    }
                )
                
                if child_created:
                    self.stdout.write(f'    ✓ Підкатегорія: {child.name}')
                    created_count += 1
        
        self.stdout.write(self.style.SUCCESS(f'\n✓ Завершено! Створено/оновлено категорій: {created_count}'))

